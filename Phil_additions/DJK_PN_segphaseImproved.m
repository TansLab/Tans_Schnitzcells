...
L2min = imextendedmin(phsegFillgaussian,5) ;   
L2 = L2min & imdilate(mask_2_bigger, strel('disk',p.L2_increase_mask_2_bigger)); figure, imagesc( L2)
 %seeds

savePNGofImage(phsegFillgaussian,'L2_phseg',p);
savePNGofImage(L2,'L2',p);
savePNGofImage(L2min,'L2min',p);
...
%--------------------------------------------------------------------------
  ...
  L4 = imfill(mesub_mask,find(L2min));

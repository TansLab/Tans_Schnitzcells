...
BG3=imread(bg3Dir);
BG3=imresize(BG3,2); %%%2x2 binning
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
quit_now=0;
global pos Limage ourfig res pp phfig flfig
...
% % nitzan's changes June25th - commenting all of this as it is obsolete.
% % JCR added to get fullsize of phase images (needed here and in expand_segimage)
% % Note this assumes all the phase images are the same size (I think that's safe)
% mynum = str3(p.manualRange(1));
...
while loopindex <= length(p.manualRange);
  ...
  if (exist('Lc')~=1 | backwards | p.override==1 | (exist('Lc')==1 & tempsegcorrect==1)) & ~mod(i-1,p.Dskip)
    ...
    %         LNedge = zeros(size(LNsub));
    %         for ie = 1:max2(LNsub);        
    %             %disp(['Cell # ',num2str(i),' edge']);
    %             LNedge = LNedge | bwperim(LNsub==ie);
    %         end;
    %         LNedge=double(+LNedge);
    %         LNedge(1:10,1:10)=1.5;
    %         imshow(makergb(+imresize(LNedge,res),imresize(phsub(:,:,1),res)));
    %       led=edge(imresize(LNsub,res));
    %       imshow(makergb2(imresize(phsub(:,:,2),res),led,led))
    ...
    %show fluor image...%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5ADD SJT
%     yreg = yreg;
if exist('yreg')
    g2 = yreg+500-BG3(rect(1):rect(3),rect(2):rect(4));
    minmin(g2)
    maxmax(g2)
    g2 = (g2-minmin(g2));
%     g2 = g2+0.2;
...
%     g2 = uint8(g2 * 255);
    ...
    g2RGB=ind2rgb(g2,jet(double(maxg2-minmin(g2))));
%    figure;image(yregRGB);
    SE=strel('square',3);
    perim = LNsub-imerode(LNsub,SE);
    perimRGB = ind2rgb(perim,cmap);
    g2m=~(perim>0);
    g2mRGB=repmat(double(g2m),[1,1,3]);
    g2cRGB = (g2RGB .*  (g2mRGB)) + perimRGB ;
...
%    set(phfig,'name',[name(outl+1:end-4)]);
    ...
    is_done=0;
    savelist=['''Lc'''];
%     savelist=[savelist,',''yreg'''];  %%%%%%%%%%%%%ADDED SJT

    crop_pop=0;
    while ~is_done
        [Lc,is_done,quit_now,dontsave,addtolist,crop_pop,newrect,savetemp,backwards,p.finetuneimage,gotoframenum] = ...
            manual_kant(p, LNsub, p.leftend, p.upend, g, p.finetuneimage, ...
                        rect, p.min_size, i, p.expandvalue);
...
%           disp(['backing up to loopindex = ',num2str(loopindex)]);
        ...
        if quit_now
            close(phfig);close(ourfig);close(flfig); %%ADD SJT
            return;
        end;
        if crop_pop

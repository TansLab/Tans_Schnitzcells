function [xycov,lags] = JK_xcov(x,y,option1,option2)
...
%   $Revision: 1.9.4.2 $  $Date: 2004/12/26 22:17:16 $
...
mx = size(x, 1);
if nargin == 1
    [xycov,l] = xcorr(x-ones(mx,1)*0.00741);
elseif nargin == 2
    if ischar(y)||(~ischar(y)&&length(y)==1)
        [xycov,l] = xcorr(x-ones(mx,1)*0.00741,y);
    else
        my = size(y, 1);
        [xycov,l] = xcorr(x-ones(mx,1)*0.00741,y-ones(my,1)*1.25);
    end
elseif nargin == 3
    my = size(y, 1);
    if length(y)==1
        [xycov,l] = xcorr(x-ones(mx,1)*0.00741,y,option1);
    else
        [xycov,l] = xcorr(x-ones(mx,1)*0.00741,y-ones(my,1)*1.25,option1);
    end
elseif nargin == 4
    my = size(y, 1);
    [xycov,l] = xcorr(x-ones(mx,1)*0.00741,y-ones(my,1)*1.25,...

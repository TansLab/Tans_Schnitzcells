...
function [autocorrelation_all] = JK_autocor_all(schnitzcells,branchdata,method,datatype,population_mean)
...
% Datatype is the fieldname in schnitzcell-structure for which to evaluate the autocorrealtion
% for instance 'MY'
%
%
...
for k = 1:length(branchdata(1,:)) %Loop over field names
    for i = branchdata(1,k)
        for j = branchdata(2,k)
          temp = JK_getautocor(schnitzcells,i,j,method,datatype,population_mean);
        end
%       save data under number of endschnitz
        autocorrelation_all{branchdata(2,k)} = temp;
    end
end
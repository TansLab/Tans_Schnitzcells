function [A_cropPhImage, Z_segmentedImage, ROI_segmentation] = PN_segphase_temp(imageToSegment,varargin)
...
q = struct; %highly simplified by inputParser in more recent matlab releases

numRequiredArgs = 1;
if (nargin < 1) | (mod(nargin,2) == 0)
  errorMessage = sprintf ('%s\n%s\n%s\n','Error using PN_segphase_temp:',...
      '    Invalid input arguments.','    Try "help PN_segphase_temp".');
  error(errorMessage);
end
...
%saving images
if ~existfield(q,'saveDir')                               %directory where intermediate treatment images are saved
    q.saveDir = pwd;       
end
...
C_localMinPh = imextendedmin(C_smoothPh,q.minDepth) & B_fillEdgeImage2;      %local minima in the mask      
...
Z_seedsFinal = bwareaopen(E_seedsCutShort,q.minCellArea*0.8);                 %suppress small seeds which artificially cut within cell

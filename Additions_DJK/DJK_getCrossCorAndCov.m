% Identical to Daan's DJK_getCrossCor but returns also crosscovariances and not only corsscorrelations 
%
%
...
function [branches, crossCor_composite, crossCov_composite] = DJK_getCrossCor(p, branches, fieldX, fieldY, varargin);
...
for br = 1:length(branches)
  ...
  %str1=num2str(branches_crossCov_XX(br).(sourceField_XX)(end));
  %str2=num2str(branches_crossCov_YY(br).(sourceField_YY)(end));
  %disp([sourceField_XX ' ' str1 '   ' sourceField_YY ' ' str2]);
end

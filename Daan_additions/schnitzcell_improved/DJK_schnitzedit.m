function p = DJK_schnitzedit(p,varargin);

global mousepos theimLc mainfig curschnitz dispframes curapproved;

% DJK 071207 using DJK_imshowlabel instead of imshowlabel
%
...
while ~done, % Main Loop:
    ...
    if colormode,
        theimrgb = label2rgb(theim,'DJK_hsv',[0 0 0],'shuffle'); %DJK 071207
        imshow(theimrgb);
    else
        imshow(theim);
    end;
    ...
    if w==0 %%mouse button
        ...
        switch upper(thechar),
            % the arrow keys
            case 'P'   %%% SJT move image up/down
               s(curschnitz).ceny=s(curschnitz).ceny+100;
            case 'L'   %%% SJT move image up/down
               s(curschnitz).ceny=s(curschnitz).ceny-100;

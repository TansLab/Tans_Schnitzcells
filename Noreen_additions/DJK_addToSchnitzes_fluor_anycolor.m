...
if strcmp(p.fluorcolor,'none')==1
    ...
    reg2=genvarname([p.fluorcolor 'reg5']);           % e.g. yreg2 
    reg3=genvarname([p.fluorcolor 'reg5']);           % e.g. yreg3 
    reg4=genvarname([p.fluorcolor 'reg5']);           % e.g. yreg4 
    ...
    for num = 1:length(trackRange)
      ...
      if existfluor==0
        ...
        eval(['existfluor=exist(''' reg4 ''');']) % check if deconvolved picture exists (=1)
        if (existfluor==1)
      ...
      for i = nonZeroSchnitzes
        ...
        if existfluor==1
          ...
          eval(['existfluor=exist(''' reg4 ''');']) % check if fluorescence picture exists (=1)
          if existfluor==1
    ...
    for i = 1:length(schnitzcells)
      ...
      eval(['existmyfield=isfield(schnitzcells, ''' fluor4_mean_all ''');']) % check if field exists (=1)
      if existmyfield==1

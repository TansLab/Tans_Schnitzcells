...
function cutcell = DJK_breakcell(cell, phcell, maxthresh, minthresh, mincelllength, figs, thinx, thiny)
% function cutcell= breakcell(cell, phcell, maxthresh, minthresh, mincelllength, figs, thinx, thiny)
%
% Copied from breakcell.
%
    ...
    if ~isempty(cutpts{k})
        ...
        for i= 1:length(cutpts{k})
            ...
            if noperims > 1
                ...
                % carry out cut 
                subcell= drawline(currcell, [cutperim1x(1) cutperim1y(1)], [cutperim2x(1) cutperim2y(1)], 0);
                subcell= bwlabel(subcell, 4);
                %disp([num2str(cutperim1x(1)) ' ' num2str(cutperim1y(1))]);
...
% figure output
if figs == 1
    figure(50); title(cellno);
    
    % plot data along thin
    subplot(2,1,1);
    ...
    % plot cell with outlint, thin, cutpoint
    subplot(2,2,4);
    subcell_ori = cell(xmin:xmax, ymin:ymax);
    subcell_ori(find(thin>0)) = 2;
    %[px, py]= tracecontour(subcell_ori);
    %for i=1:length(px), subcell_ori(px(i),py(i)) = 3; end
    perim= bwperim(imdilate(subcell, strel('disk',1)));
    pxy = find(perim>0);
    for i=1:length(pxy), subcell_ori(pxy(i)) = 3; end
    subcell_ori(fx(cutpts{1}),fy(cutpts{1})) = 4; 
    subcell_ori(cutperim1x,cutperim1y) = 5; 
    subcell_ori(cutperim2x,cutperim2y) = 5; 
    imshowlabel(subcell_ori);

function [L, phsub, L9, rect, L8, L7, L6, L5, L4, L3, mesub]= DJK_segphase(ph3, p)
% function [L, phsub, L9, rect, L8, L7, L6, L5, L4, L3, mesub]= DJK_segphase(ph3, p)
% 
...
% ph3_preFilt=ph3; %ST %DJK 071120
%imwrite(ph3(:,:,p.segmentationPhaseSlice) ,[p.segmentationDir,p.DJK_saveDir,'ph3_before','.tif'], 'TIFF', 'Compression', 'none'); %DJK 081211
for i= 1:size(ph3,3),
...
%imwrite(ph3(:,:,p.segmentationPhaseSlice) ,[p.segmentationDir,p.DJK_saveDir,'ph3_after','.tif'], 'TIFF', 'Compression', 'none'); %DJK 081211
%keyboard;
 me=edge(ph3(:,:,p.segmentationPhaseSlice),'log',0,p.edge_lapofgauss_sigma);%o
%rigJB
% me= edge(ph3(:,:,p.segmentationPhaseSlice),'log',0,1); %ST
col_center=[size(me,1),size(me,2)]/2;

DJK_savePNG(me,'ME',p); %DJK 071122
...
for x= 1:replaceboxsize:size(me,1) - boxsize
    for y= 1:replaceboxsize:size(me,2) - boxsize
        ...
        checkiscell= sum(subregion(:)) < p.minNumEdgePixels; %orig JB
%         checkiscell= sum(subregion(:)) < 150; %ST
...
DJK_savePNG(imout,'IMOUT',p); %DJK 071122
...
r= regionprops(imoutlabel, 'area', 'centroid');
    ...
    % figure; image(imout,'CDataMapping','scaled');
    % keyboard;
...
%figure; image(phsub,'CDataMapping','scaled');
...
DJK_savePNG(mask,'mask',p); %DJK 071126
...
while use_mask_cutting<3;
    ...
    DJK_savePNG(bigmask,'BIGMASK',p); %DJK 071122
    DJK_savePNG(mesub,'MESUB',p); %DJK 071122
    DJK_savePNG(bigmesub,'BIGMESUB',p); %DJK 071122
    ...
    for i= 1:p.numphaseslices 
        ...
        DJK_savePNG(ebot,['L1_ebot_' num2str(i)],p); %DJK 071123
        ...
        DJK_savePNG(ebot,['L1_ebotb_' num2str(i)],p); %DJK 071123
        
        if p.useBigMask %DJK 071126
            ebot2= imfill(ebot,'holes') & bigmask; %DJK 071126
        else %DJK 071126
            ebot2= imfill(ebot,'holes') & mask;
        end %DJK 071126
        % remove regions between cells from L
        
        DJK_savePNG(ebot2,['L1_ebot2_' num2str(i)],p); %DJK 071123
    ...
    DJK_savePNG(L2,'L2',p); %DJK 071120
...
DJK_savePNG(L3,'L3',p); %DJK 071120
...
%DJK_savePNG(mesub,'MESUB_later',p); %DJK 081210
%disp(['max2(L3) is ' num2str(max2(L3))]); %DJK 081210
for i= 1:max2(L3)
    ...
    %DJK_savePNG(L3i,'L3i',p); %DJK 081210
...
DJK_savePNG(L4,'L4',p); %DJK 071120
...
DJK_savePNG(L5,'L5',p); %DJK 071120
...
DJK_savePNG(L6,'L6_despeckled',p); %DJK 071127

% START DJK 071127
% Added to first cut up cells with smaller maxCellWidth
% Is the same as part below, but now using maxCellWidthConservative
if p.maxCellWidthConservative < p.maxCellWidth
    r= regionprops(L6, 'solidity');
    fkinks= find(([r.Solidity] < 0.85));
    disp(['Cutting individual cells(',num2str(length(fkinks)),') conservatively (DJK).']);
    for i= 1:length(fkinks)
        %disp([' ',num2str(i),':cutting cell number ',num2str(fkinks(i))]);
        Lcell= (L6 == fkinks(i));
        % save(['Lcell_' num2str(i)],'Lcell') %DJK 071126
        cutcell= cutcurv(Lcell, p.maxThreshCut2, p.maxCellWidthConservative, p.minCellLength);
        cellnos= unique(cutcell);  
        L6(Lcell)= 0;
        label= max2(L6);
        for j= 2:length(cellnos)
    ...
    L6= renumberimage(L6);
    DJK_savePNG(L6,'L6_conservative',p); %DJK 071127
end
% END DJK 071127
...
DJK_savePNG(L6,'L6',p); %DJK 071120
...
fbiggies= find(([r.MajorAxisLength]>70)); % DJK 071127 was 50, now 70
...
DJK_savePNG(L7,'L7',p); %DJK 071120
...
if ~p.noKinky %DJK 071127
    ...
    L8= renumberimage(L8);
    DJK_savePNG(L8,'L8',p); %DJK 071120
end %DJK 071127
...
L9= carefuldilate(+L8, strel('diamond',1), 2); %ST
%L9= carefuldilate(+L8, strel('diamond',1), 1); %ORIG
...
DJK_savePNG(L9,'L9',p); %DJK 071120

DJK_savePNG(L,'L',p); %DJK 071120
...
function DJK_savePNG(LL, name, p)
    if isfield(p,'DJK_saveDir')
        if isempty(LL) %DJK 080219
            LL = [ 0 ]; %DJK 080219
        end %DJK 080219
        DJK_writeSegImage(LL,[p.segmentationDir,p.DJK_saveDir,name,'.png']); %DJK 081210
        % imwrite(LL,[p.segmentationDir,p.DJK_saveDir,name,'.png'],'png'); %DJK 071120
        disp(['Written ',p.segmentationDir,p.DJK_saveDir,name]); %DJK 071120
    end
% END DJK 071120

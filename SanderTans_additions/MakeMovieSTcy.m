...
moviefile = [p.imageDir,'PhFyc2.avi'];
moviefile
p.imageDir
...
%     imy = (imy - FyOffset - BG); %% ONLY IF BACKGRND SUBTRACT!!!
    ...
    imc = imread([basepathFc,str3(i),'.tif']);    
%     imc = (imc - FyOffset - BG); %% ONLY IF BACKGRND SUBTRACT!!!
    imc = (imc - FyOffset); %% ONLY IF BACKGRND SUBTRACT!!!
    i
    maxmax(imy)
    minmin(imy)
    maxmax(imc)
...
%   imshow(makergb(imy,imc));
   imshow(makergb(imresize(imp,0.5)-PhOffset,imy,imc));
%     keyboard
    F = getframe(gca);
    mov = addframe(mov,F);
end

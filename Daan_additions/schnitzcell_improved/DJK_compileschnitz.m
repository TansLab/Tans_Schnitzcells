function [p,schnitzcells] = DJK_compileschnitz(p,varargin) 
%   DJK_COMPILESCHNITZ given a lineage, suck out fluor info & calc area, length, etc
% 
%   Same as COMPILESCHNITZ but with these changes:
%
%   1. Extra option for Single Cell analysis, where no data is extrapolated 
%   between frames (all the delta data). These do not make sense in Single 
%   Cell analysis, as a cell is never tracked for more than 1 frame. 
%   'singleCellMode' flag, if true, will not do the extrapolation.
%
%   2. A different micronsPerPixel setting (=0.04065), corresponding to
%   the 1.5x magnification of the Killer Mike.
%
%   3. Normalization of fluorescence (division by background) can be turned
%   of, by flagging 'normalization' false.
%
%   COMPILESCHNITZ allows users to extract fluorescence info from a cell 
%   lineage tree that describes how cells are tracked across frames of a 
%   movie. The fluorescence info is attached to the tree, and the resulting 
...
if numExtraArgs > 0
  for i=1:2:(numExtraArgs-1)
    ...
    fieldName = DJK_schnitzfield(varargin{i}); % DJK 080704
...
if ~existfield(p,'singleCellMode') % DJK 071022
  p.singleCellMode = 0;
end
if ~existfield(p,'normalization') % DJK 071114
  p.normalization = 1;
end
...
if ~p.singleCellMode % DJK 071022
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function xreg = normalizeF( xreg, ...                        % gfp image in camera units
                            xback, ...                       % background in camera units
                            zzxim, ...                       % ? from flatfield
                            rect, ...                        % area of LC in original crop
                            exptx, ...                       % exposure time
                            Auto_XFL, ...                    % ? p.yfpOffset
                            camera_bin_size, ...             % binning: 2
                            XFPmolecules_per_XFLunit, ...    % ? from flatfield, # of molecules per camera unit
                            crosstalkin, ...                 % ? crosstalking image ?
                            crosstalkratio)               % ? p.crosstalkRatioC2Y
                            
xreg=double(xreg)-xback; % in camera_xfp_units; xback also in camera units.
if prod(size(zzxim))>10 % if correction image is bigger than 3 x 3
    xreg=xreg./zzxim(rect(1):rect(3), rect(2):rect(4));
end
% exptx=1; %SJT
if (exptx=='empty') exptx = 1; end % DJK 071114
xreg=xreg/exptx; % in camera_xfp_1sec_units;
xreg=xreg*XFPmolecules_per_XFLunit;
xreg=xreg/(camera_bin_size^2);
xreg=xreg-Auto_XFL; % in units of molecules?
...
ncells= cells;
% find cell
% SJT addditions to make sure Lc has the same size as yreg to avoid dips:
xdmin=min(size(yreg,1),size(Lc,1));
ydmin=min(size(yreg,2),size(Lc,2));
Lc=Lc(1:xdmin,1:ydmin);
yreg=yreg(1:xdmin,1:ydmin);
% creg=creg(1:xdmin,1:ydmin);
% end of addition
...
if ~isempty(yreg) | ~isempty(yreg),
    ...
    %AA interference for solving crazy low peaks:
%     figure
%     image(Lc,'CDataMapping','scaled')
%     figure
%     MoviePerim
%     loc
%     yreg(loc)

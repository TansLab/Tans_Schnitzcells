function p = tracker_tps(p,varargin)
%   
...
seq_out_name = [p.tracksDir, p.movieName, '_tracks'];
fid_out = fopen([seq_out_name '.txt'],'w');
...
for frameNum = p.trackRange(2:end)
    ...
    yesterdaySegFile = [p.segmentationDir, outprefix, str3(yesterdayFrameNum)];
...
%    end
    ...
    switch (str)
        case 'PCWIN'
            trackCommand = [trackCommand,' > ',logFile,' 2>&1'];
        otherwise
            trackCommand = [trackCommand,' >& ',logFile];
    end
...
%        disp(['Matching frame pair ',mynum_y,' , ',mynum_t,'...']);

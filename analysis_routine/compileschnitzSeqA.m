...
schnitzcells= add_SeqA(schnitzcells, p.micronsPerPixel);%AA addition
...
if ~isempty(creg),
    figure(cfig);
    imshow(makergbSTdot(creg,(Lc==cellnum)));
...
%     keyboard
    ...
    ncells(schnitzno).NdotC(age)= Ndot;
    for i=1:4
        if i<=Ndot        
%             [Ndot i]
            ncells(schnitzno).dotx(age,i)= X(i); 
            ncells(schnitzno).doty(age,i)= Y(i);
        else
            ncells(schnitzno).dotx(age,i)= NaN;
            ncells(schnitzno).doty(age,i)= NaN;
        end
    end
    %Xdot=schnitzcells(1,1).dotx
    %XdotCen=Xdot-repmat(schnitzcells(1,1).cenx',1,4)
    %
    %Ydot=schnitzcells(1,1).doty
    %YdotCen=Ydot-repmat(schnitzcells(1,1).ceny',1,4)
    
%     figure
%     plot(XdotCen,YdotCen)
%     plot(Xdot,Ydot)
...
%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function newschnitzcells = add_SeqA(schnitzcells,spotnum)%AA addition

for i = 1:length(schnitzcells),
   
    schnitzcells(i).SeqA = schnitzcells(i).len;
    %to see how long the array should be, look at the value of N
    %schnitzcells(1).SeqA = [0 0 0 0 1 1 1 2 2 2 2 1 0];

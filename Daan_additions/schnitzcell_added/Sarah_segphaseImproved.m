function [phsub, L10, rect, phsegsub] = DJK_segphaseImproved(ph3, p)
% function [phsub, L10, rect, phsegsub] = DJK_segphaseImproved(ph3, p)
% 
...
%--------------------------------------------------------------------------
% Process:
%   * phsegFill is mean of slices in ph3 indicated in p.fillingEdgeSlices
%   * perform bottom hat algorithm on phsegFill
%   * perform edge on this image
%   * fill this edge, and use as seed later
...
phbot = imbothat(double(phsegFill).*double(mask_2_bigger), strel('disk',p.botHatSize));

% DJK 090614, sometime need bigger mask to avoid cells at edge being open
% to background. Is set using p.mask_2_bigger_dilate, which default = 0. (Original was 0)
ebot = edge(phbot,'log',0) & imdilate(mask_2_bigger, strel('disk',p.L2_increase_mask_2_bigger));

% add lines to fill cells at the edge of the field of view.
be = zeros(size(mask_2_bigger));
be(1,1:end) = 1;
be(end,1:end) = 1;
be(1:end,1) = 1;
be(1:end,end) = 1;
ebot = ebot | (be & mask_2_bigger);    

% ?
if p.L1Fill_only_use_mask_2_bigger % DJK 090319: changed for 090305 pos15 fr 91->
  ebot2= imfill(ebot,'holes') & mask_2_bigger;
else
  ebot2= imfill(ebot,'holes') & mask_2; % DJK 090811 in case of error change to 'mask_2_bigger'
end

% remove regions between cells from L
L2(~ebot2)= 0;

% Renumber independent cells
L2 = bwlabel(+L2>0,4); % or L2 = bwlabel(L2,4);
        
savePNGofImage(phbot,['L2_phbot'],p);
savePNGofImage(      ebot,['L2_ebot'],p);
savePNGofImage(     ebot2,['L2_ebot2'],p);
% savePNGofImage(be,'L2_be',p);
savePNGofImage(L2,'L2',p);
clear phsegFill phbot ebot ebot2;
%--------------------------------------------------------------------------


%--------------------------------------------------------------------------
% STEP 3: REMOVE SMALL CELLS
%--------------------------------------------------------------------------
% Process:
...
for j = 1:length(flittles),
  % delete cell
  L3(L3 == flittles(j))= 0;
end;
L3 = renumberimage(L3);

savePNGofImage(L3,'L3',p);
clear r flittles;
%end
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% Process:
%   * for each cell in L3, imerode untill few pixels left
%   * use these pixels as seed in edge, and fill
%   * remove stuff 3 pixels outside mask_2

% ?
L4_mesub = mesub | (be & mask_2_bigger); % be is frame at edge of image

L4_fill_1 = zeros(size(L3));

% visualize effect of strelsize
strelsizeTemp = strelsize; %strelsize is set at SETTINGS
while strelsizeTemp>0
  strelsizeTemp = strelsizeTemp-1;
  L3temp = imerode(L3, strel('disk',strelsizeTemp));
  savePNGofImage(L3temp,['L3_imerode_' num2str(strelsizeTemp)],p);
end

% loop over cells found in L3, decrease size of cell with imerode, and if
% still something left, use it to fill L4_mesub (else decrease less)
for i = 1:max2(L3) 
  fx = []; strelsizeTemp = strelsize; %strelsize is set at SETTINGS
  
  % The previous procedure without the while loop and decreasing strelsize,
  % could miss cells if they are paired and connected by a closed-off edge
  % (i.e. do not form one cavity):
  while length(fx)==0 & strelsizeTemp>0
    strelsizeTemp = strelsizeTemp-1;
    L3i = imerode(L3==i, strel('disk',strelsizeTemp));
    [fx, fy] = find(L3i);
  end
  
  % use pixels that are left to fill L4_mesub
  tempim1 = imfill(L4_mesub, [fx, fy]);

  % remove pixels of L4_mesub
  tempim2 = tempim1 & ~L4_mesub;

  % save into L4
  L4_fill_1(tempim2 > 0) = i;
end
...
% remove stuff 3 pixels outside mask_2
if p.L1Fill_only_use_mask_2_bigger % DJK 090319: changed for 090305 pos15 fr 91->
  L4_fill_2( imdilate(mask_2_bigger, strel('disk',3))==0 ) = 0;
else
  L4_fill_2( imdilate(mask_2, strel('disk',3))==0 ) = 0; % DJK 090211 in case of error change to 'mask_2_bigger'
end
...
%savePNGofImage(L4_mesub,'L4_mesub',p);
savePNGofImage(L4_fill_1,'L4_fill_1',p);
savePNGofImage(L4_fill_2,'L4_fill_2',p);
savePNGofImage(L4,'L4',p);
clear L4_mesub L4_fill_1 L4_fill_2;
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% STEP 10: Fill up holes
%--------------------------------------------------------------------------
L10 = imfill(L9C);

savePNGofImage(L10,'L10',p);
%--------------------------------------------------------------------------

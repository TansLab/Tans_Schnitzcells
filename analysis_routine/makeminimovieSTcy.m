 function p = makeminimovieSTcy(p,varargin) 
% MAKEMINIMOVIE creates a sub-movie within the given movie around clicked points
% just like the original makeminimovie, but now for -c- and -p-1- images
%
...
while i<=imax
    ...
    imc = imread([p.imageDir,p.movieName,'-c-',str3(frame),'.tif']);
%     imy = imy + 600 - BG;
...
%     imshow(makergb(imresize(imp,0.5),imy));
    figure(origfig);
    imshow(makergb(imresize(imp,0.5),imc));
...
%         [p.winSizeX p.winSizeY]
        ...
        newimc=imc(y1h(i):y2h(i),x1h(i):x2h(i));
         [frame x2h(i)-x1h(i) y2h(i)-y1h(i)]
        
        figure(reducefig);
        imshow(makergb(imresize(newimp,0.5),newimy));
%         figure(reducefig2);
%         imshow(makergb(imresize(newimp,0.5),newimc));
...
basepathFc = [p.imageDir,p.movieName,'-c-'];
outpathFc = [newp.imageDir,newp.movieName,'-c-'];
% frames
for i = 1:size(clickdata,1),
    ...
    imc = imread([basepathFc,str3(frames(i)),'.tif']);  
%     imy = imy + 600 - BG;
    imyi = imfinfo([basepathFy,str3(frames(i)),'.tif']);
    imci = imfinfo([basepathFc,str3(frames(i)),'.tif']);
    ...
    descrc=[imci.ImageDescription 'DateTime: ' imci.DateTime 'Software: ' imci.Software];
    ...
    imwrite(newimc,[outpathFc,str3(frames(i)),'.tif'],'tif','Description',descrc);
end;
close(origfig);
close(reducefig);
...
for i= p.segRange
  ...
  [phsub,LNsub,rect,phseg]= DJK_segphaseImproved(ph3, p);
  ...
  % Prepare fluor2 data for saving
  ...
  if exist(rname)==2 & numel(rect)>0 % if no cells found, rect is empty, and gives error
    ...
    [rreg, rshift, rback, rbinning] = quicknoreg(LNsub,rname,rect,0,phaseFullSize); % not really important, cause redone during DJK_correctFluorImage
